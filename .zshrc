...
typeset -A zplugins
# Proper initialization for associative arrays
zplugins[git]=true                     # Enable Git plugin features
zplugins[zoxide]=true                  # Enable zoxide (intelligent cd)
zplugins[syntax-highlighting]=true     # Enable zsh-syntax-highlighting
zplugins[autosuggestions]=true         # Enable zsh-autosuggestions
zplugins[history-substring-search]=true # Enable history substring search
zplugins[archlinux]=true               # Enable Arch Linux specific aliases/functions (if applicable)
...
_install_if_missing() {
  ...
  local custom_installer="$HOME/git/conf/install.sh" # Use full path
...
# Can be disabled with SKIP_ZIMFW=1 environment variable
if [[ "$SKIP_ZIMFW" != "1" ]]; then
...
# Theme (optional - uncomment if desired)
...
EOF
  ...
  _log_message "INFO" "ZimFW disabled via SKIP_ZIMFW environment variable"
fi
